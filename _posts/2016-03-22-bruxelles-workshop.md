 ...
 * Or may be restart with Aquilon if it is ready for production with the template library
...
* Clojure migration may have helped but will no append in a foreseeble feature
...
* No list available in the doc : look at the documentation
...
* Duplication installation documentation: repo README and quattor.org documentation
...
* Order of feature inclusions is not guaranteed but 2 stages (pre and post) in the inclusion process
  ...
  * Including featres in others features means that their usage cannot be tracked by Aquilon: normally they are added under /metadata in the profile
...
* Agreeement to do the necessary changes and cleanup the template names to be devices rather than driver names
...
* In the short term, would be good to allow to disable some part of the config in the profile (for example configuration of a specific OSD)
...
* Support for all core services available, including Cinder, Ceilometer
  ...
  * No specific OpenStack tree in the config
  
VM management with Quattor

* LAL way: a very basic image with a contextualization script that triggs Quattor installation and configuration
  * No impact on Quattor except removing network information from the profile
  * RAL doing a similar approach: goal is to hide the configuration system from the user provisionning the resource
...
Cloud resource provisionning: RAL integrated it into their cloud dashboard 
...
* Objective: makes components using CAF modules to be able to report about what was done by CAF
  ...
  * Will also allow to remove files created by Quattor when a component is removed if desirable
* CAF::History allows to build a list of event that can be processed later
  ...
  * Nathan would prefer immediate reporting of the events rather than a deferred one (in case a component crashes before reporting occured): improve current implementation to track if an event has already been reported and makes possible to call the report method several times still reporting event once. A global property could be taken into account to decide wehn an event is added to the list whether immediate reporting should occur.
...
* Agreement that the behaviour exhibited in the issue is incorrect
* Test the PR against the template library and checks that it produces no difference (that nothing depends on the wrong behaviour
...
* Agreement that something should be done, proposed approach reasonnable but ncm-fstab may become quite misleading
...
* latest contains the last version of configuration-modules-core, ccm and a few other things
  ...
  * schema parsed to retrieved type of properties and wheter they are required or optional (may want to add the default value in the future)
